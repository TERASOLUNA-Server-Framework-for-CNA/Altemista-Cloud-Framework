/**
 * 
 */
package org.altemista.cloudfwk.it;

/*
 * #%L
 * altemista-cloud cache: Ehcache integration tests
 * %%
 * Copyright (C) 2017 NTT DATA Corporation
 * %%
 * All rights reserved.
 * Todos los derechos reservados.
 * #L%
 */


import org.junit.Test;
import org.altemista.cloudfwk.it.AbstractIT;
import org.altemista.cloudfwk.it.cache.controller.ApplicationCacheableResourceController;

/**
 * Simple, base integration test to check the altemista-cloudfwk-core-cache module.
 * @author NTT DATA
 */
public class ApplicationCacheableResourceIT extends AbstractIT {
	
	/**
	 * Validates all the controller options
	 */
	@Test
	public void tests() {		
		this.testMappings(ApplicationCacheableResourceController.MAPPING);
	}
	
}
