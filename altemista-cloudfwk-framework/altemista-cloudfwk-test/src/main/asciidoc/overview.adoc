...
Though originally developed for internal use only, the `altemista-cloudfwk-test` artifact is published in order to both simplify the `test` scoped dependencies, and to provide some utility classes to the applications.
...
----
include::{basedir}/altemista-cloudfwk-test/src/test/java/org/altemista/cloudfwk/test/SpringContextTest.java[tags=example]
----
...
.SpringContextTest.java
----
include::{basedir}/altemista-cloudfwk-test/src/test/java/org/altemista/cloudfwk/test/CustomSpringContextTest.java[tags=example]
----
